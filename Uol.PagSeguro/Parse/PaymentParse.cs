    ...
    internal static class PaymentParse
    {
        ...
        public static IDictionary<string, string> GetData(PaymentRequest payment)
        {
            ...
            if (payment.PreApproval != null)
            {
                ...
                data["reviewURL"] = payment.PreApproval.ReviewUrl;

                if (payment.PreApproval.Period == "YEARLY")
                {
                    data["preApprovalDayOfYear"] = payment.PreApproval.DayOfYear;
                }

                if (payment.PreApproval.Period == "MONTHLY" || payment.PreApproval.Period == "BIMONTHLY" || payment.PreApproval.Period == "TRIMONTHLY")
                {
                    data["preApprovalDayOfMonth"] = payment.PreApproval.DayOfMonth;
                }

                if (payment.PreApproval.Period == "WEEKLY")
                {
                    data["preApprovalDayOfWeek"] = payment.PreApproval.DayOfWeek;
                }
            }
